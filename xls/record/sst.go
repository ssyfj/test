...
/*
The SST record contains string constants. SST记录包含字符串常量。
...
4 			cstTotal 	4 			Total number of strings in the shared string table and
									extended string table ( EXTSST record)	共享字符串表和扩展字符串表中的字符串总数（EXTSST记录）
8 			cstUnique 	4 			Number of unique strings in the shared string table	共享字符串表中的唯一字符串数
12 			rgb 		var 		Array of unique unicode strings (XLUnicodeRichExtendedString).唯一unicode字符串的数组（XLUnicodeRichExtendedString）。
...
func (s *SST) NewSST(buf []byte) { //0-4，4-8，8->...
